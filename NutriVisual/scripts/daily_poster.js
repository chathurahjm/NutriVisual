...
// Helper to load pre-generated social copy dataset
function loadSocialPosts() {
  const postsPath = path.join(__dirname, '../src/data/social_posts.json');
...
// Generate social post copy (Pre-generated dataset -> AI APIs -> Local template fallback)
async function generatePostCopy(food1, food2) {
  const pairKey = `${food1.id}-vs-${food2.id}`;
  const socialPosts = loadSocialPosts();

  // 1. Primary: Use pre-generated copy from social_posts.json if available
  if (socialPosts[pairKey]) {
    console.log(`📖 Using pre-generated copy from dataset for ${pairKey}...`);
    return socialPosts[pairKey];
  }
...
`;
  ...
  // 3. Fallback Template
  console.log('⚠️ Using local fallback template copy.');
  return {
